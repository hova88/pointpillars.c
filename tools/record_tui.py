...
def load_fonts(size: int) -> tuple[ImageFont.FreeTypeFont, ImageFont.FreeTypeFont,
    ...
    braille_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
    if not pathlib.Path(braille_path).exists():
        braille_path = regular_path
...
def termios_tiocswinsz() -> int:
    # Linux TIOCSWINSZ; termios does not expose it on every Python build.
    return 0x5414
